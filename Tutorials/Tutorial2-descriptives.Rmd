...
A mean value in circular data is referred to as the *mean direction*. For example, the mean direction in a dataset with observations on a compass pointing north (\(0 ^\circ\)) and east (\(90 ^\circ\)) would be northeast (\(45 ^\circ\)). Although intuitively this makes sense, it often is not as straightforward to calculate as the mean in linear data. Unlike in linear data, it is not possible to take the sum of all values and divide them by $n$. Imagine the same scenario as before but with the compass indicating \(359 ^\circ\) instead of (\(0 ^\circ\)). Where a linear mean would drastically shift, the mean direction will barely change. 

To obtain the mean direction then, requires us to use the sum of cosines (\(C = \sum \text{cos} \theta_i \)), sum of sines (\(S = \sum \text{sin} \theta_i \)) and \(R = \sqrt{C^2 + S^2} \), which is the square root of the sum of squared $C$ and $S$ values. The mean direction is then given by:
...
As we can see, the mean and the mean direction of values in degrees are very different. Where the regular mean simply takes the sum of all values and divides them by the amount of values, the mean direction takes account of the fact that the data is defined on a circular continuum. In the circular dataset, the datapoints \(90 ^\circ\) and \(270 ^\circ\) are both the same distance from \(0 ^\circ\). Finally, the values of \(0 ^\circ\) and \(360 ^\circ\) are identical. The resultant mean direction is `r round(mean_deg, 1)`\(^\circ\), which is equal to `r round(mean_deg %% 360, 1)`\(^\circ\).
...
Since the mean resultant length is a measure of concentration and the variance is traditionally a measure of spread, the variance in circular data is simply 1 minus the mean resultant length. Its interpretation is opposite to that of the mean resultant length The function `var.circular` will give you this value, but it is also easily obtained by \(1 - \rho\). The standard deviation is *not* obtained using the square root of the variance, but rather using \((-2 \log\ (1-var))^{1/2}\).
