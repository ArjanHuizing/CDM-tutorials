...
An example would be the time of day as represented on the clock. As time passes the hands on the face of a clock will inevitably be bound to pass the same 12 hours over and over. This also means that distances can not be interpreted as linear. The difference in time between 1 o'clock and 3 o'clock is the same as the distance between 23 o'clock and 1 o'clock, that is, two hours. Another quality of circular data is that every value can be seen to have an opposite value. A second instrument that provided circular measurements is the compass. When we look at directions on a compass, we would say that north is the opposite of south, and east is the opposite of west. Circumplex models also use this quality to create a continuum of related values, where opposite values are 180 degrees apart. Examples of this from social and behavioural science are Schwartz' theory of basic values, Russel's mood scale and Leary's interpersonal circumplex.
